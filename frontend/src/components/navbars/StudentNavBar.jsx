...
const StudentNavBar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-[#FFFFFF] border-b border-[#F5F6FA] font-['Nunito'] shadow-sm box-border transition-all duration-300">
      <div className="px-6 py-4 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-5">
        <div className="flex flex-wrap items-center gap-2 md:gap-3 text-sm font-medium flex-1">
          <Link
            to="/student/dashboard"
            className="ml-4 text-[23px] font-black tracking-tighter text-[#6C5CE7] mr-6 whitespace-nowrap no-underline flex items-center transition-all duration-300 hover:opacity-80"
          >
            <img src={Logo} alt="InternStatus" className="h-10" />
            <span className="text-[14px] font-bold text-[#2D3436] opacity-50 ml-4 tracking-wide uppercase">
              Student
            </span>
          </Link>
        </div>
